...
main = Net.withSocketsDo $ do
	let fromString portString = Net.PortNumber . fromInteger . read $ portString
	args <- getArgs
	case args of
		[host, port, "--get", k] -> do
			server <- Net.connectTo host (fromString port)
		...
		[host, port, "--test"] -> do
			...
			threads <- sequence [forkIO (testWith lock host (fromString port)) | lock <- locks]
			sequence_ [takeMVar lock | lock <- locks] -- Wait for threads to finish
			putStrLn "Test complete."
		_ -> do
			putStrLn "Usage: Client <host> <port> <args>"
			putStrLn "<args> are \"--test\", \"--get <key>\", or \"--set <key> <value>\""
...
testWith :: MVar () -> String -> Net.PortID -> IO ()
testWith lock host port = do
